class Accion():
  def __init__(self, planta, maceta, posicion, semana):
    ...
    self.semana = semana
  def __hash__(self):
    return hash((self.planta, self.maceta, self.posicion, self.semana))
  ...
  def se_superpone_con(self, otra):
    ...
    for semana_desde_plantada, tamanio_planta in enumerate(self.planta.crecimiento):
      ...
      print('Semana bajo analisis', semana_bajo_analisis)
      ...
      print('Tamanio', tamanio_planta, 'Otra', tamanio_otra)
      ...
      print('Bordes planta', bordes_planta, 'Otra', bordes_otra)
...
def se_superponen_en_eje_horizontal(bordes, bordes_otra):
  borde_izq, borde_der, borde_sup, borde_inf = bordes
  otra_borde_izq, otra_borde_der, otra_borde_sup, otra_borde_inf = bordes_otra
  return borde_izq <= otra_borde_der and borde_der >= otra_borde_izq or \
          borde_der >= otra_borde_izq and borde_izq <= otra_borde_der

def se_superponen_en_eje_vertical(bordes, bordes_otra):
  borde_izq, borde_der, borde_sup, borde_inf = bordes
  otra_borde_izq, otra_borde_der, otra_borde_sup, otra_borde_inf = bordes_otra
  return borde_sup <= otra_borde_inf and borde_inf >= otra_borde_sup or \
          borde_inf >= otra_borde_sup and borde_sup <= otra_borde_inf